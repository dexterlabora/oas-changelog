...
export default function swaggerDiff(oldSpec, newSpec, config) {
  console.log("diff config", config);
  const debug = require("debug")("swagger-diff:workflow");
  debug("start");

  config = getConfig(config);
  return Promise.all([prepareSpec(oldSpec), prepareSpec(newSpec)]).then(
    ([prepOldSpec, prepNewSpec]) => {
      ...
      if (prepOldSpec.info && prepNewSpec.info) {
        ...
        if (!semver.valid(oldVersion) || !semver.valid(newVersion)) {
          debug(
            "one swagger file version is not semver compliant => ignore version comparison"
          );
        } else {
      ...
      const diffs = config.skipDiffPostProcessing
        ? changes
